...
const ClientArea = () => {
  ...
  if (isLoading) {
    return (
      <section id="cliente" className={`min-h-screen flex flex-col justify-center py-4 px-4 md:py-20 md:px-16 lg:px-24 ${isDark ? 'bg-black' : 'bg-white'}`}>
  ...
  return (
    <section 
      id="cliente" 
      className={`h-full flex flex-col justify-center items-center py-4 px-4 md:py-20 md:px-6 lg:px-24 ${isDark ? 'bg-black' : 'bg-white'} ${isDark ? 'text-white' : 'text-black'}`}
      style={{ minHeight: '100vh' }}
    >
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col md:flex-row items-center gap-4 md:gap-10 lg:gap-16">
          <div className="w-full md:w-2/5 order-1 md:order-1">
            <div className="relative mb-4 md:mb-0 bg-black p-3 md:p-8 rounded-2xl md:rounded-3xl shadow-2xl transform transition-all duration-500 hover:scale-105">
          ...
          <div className="w-full md:w-3/5 text-center md:text-left order-2 md:order-2">
            <h2 
              ref={titleRef} 
              className={`text-lg md:text-2xl lg:text-3xl xl:text-4xl 2xl:text-5xl mb-3 md:mb-6 font-canela ${isDark ? 'text-white' : 'text-black'}`}
            >
            ...
            <p 
              ref={textRef} 
              className={`text-sm md:text-lg lg:text-xl mb-4 md:mb-8 font-satoshi ${isDark ? 'text-gray-300' : 'text-gray-700'}`}
            >
            ...
            <div className="flex flex-col space-y-2 md:space-y-4">
              <a 
                ...
                className={`group relative overflow-hidden rounded-lg md:rounded-xl ${isDark ? 'bg-white text-black' : 'bg-black text-white'} px-4 md:px-8 py-2 md:py-4 transition-all duration-300 hover:shadow-lg flex items-center justify-center text-sm md:text-base`}
              >
                ...
                <ArrowRight className="ml-1 md:ml-2 h-4 w-4 md:h-5 md:w-5 relative z-10 transition-transform duration-300 group-hover:translate-x-1" />
              </a>
              ...
              <a 
                ...
                className={`group relative overflow-hidden rounded-lg md:rounded-xl border ${isDark 
                  ...
                  : 'border-black/20 text-black'} px-4 md:px-8 py-2 md:py-4 transition-all duration-300 hover:shadow-lg flex items-center justify-center text-sm md:text-base`}
              >
