...
const Partners = () => {
  ...
  return <div ref={sectionRef} className={`h-full w-full py-4 px-4 md:px-8 lg:px-16 ${isDark ? 'bg-black text-white' : 'bg-[#f5f5f5] text-black'} relative`} style={{
    minHeight: '100vh',
    display: 'flex',
    flexDirection: 'column',
    justifyContent: 'center',
    marginTop: isMobile ? '-100px' : '-150px',
    // Subiu mais 50px no desktop
    paddingTop: '50px'
  }}>
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10">
        <div className="flex flex-col items-center justify-center flex-1">
          ...
          <div className="relative w-full max-w-6xl px-4 sm:px-8 lg:px-12">
            <div ref={carouselRef} className="overflow-hidden">
              ...
              width: `${totalSlides * 100}%`
                ...
                width: `${100 / totalSlides}%`
                    ...
                    {teamMembers.slice(slideIndex * itemsPerSlide, (slideIndex + 1) * itemsPerSlide).map((member, index) => <div key={index} className={`group p-2 sm:p-3 lg:p-4 ${isMobile ? 'w-full max-w-sm' : ''}`}>
                          <div className={`relative overflow-hidden rounded-lg transition-all duration-300 hover:scale-105 hover:-translate-y-2 ${isDark ? 'bg-white/5 hover:bg-white/10' : 'bg-white hover:bg-gray-50'} shadow-md hover:shadow-xl`}>
            ...
            {totalSlides > 1 && <div className="flex justify-center mt-6 sm:mt-8 space-x-1.5">
                {Array.from({
              length: totalSlides
            }).map((_, index) => {})}
              </div>}
