        ...
        {displayPosts.length > 0 ? <>
            ...
            <div className="relative w-full max-w-6xl mx-auto px-4 sm:px-8 lg:px-12 mb-8">
              ...
              width: `${totalSlides * 100}%`
                  ...
                  {Array.from({
                length: totalSlides
              }).map((_, slideIndex) => <div key={slideIndex} className={`w-full flex-shrink-0 px-2 sm:px-4 ${isMobile ? 'flex justify-center' : isTablet ? 'grid grid-cols-2 gap-4 sm:gap-6' : 'grid grid-cols-3 gap-4 sm:gap-6 lg:gap-8'}`} style={{
                      ...
                      {displayPosts.slice(slideIndex * itemsPerSlide, (slideIndex + 1) * itemsPerSlide).map(post => <div key={post.id} className={`group p-2 sm:p-3 lg:p-4 ${isMobile ? 'w-full max-w-sm' : ''}`}>
                            <Card className={`cursor-pointer transition-all duration-300 hover:scale-105 backdrop-blur-sm border flex flex-col ${isTablet ? 'h-80' : 'h-96'} ${isDark ? 'bg-neutral-900/80 border-neutral-800/50 hover:border-neutral-700/60 shadow-lg hover:shadow-2xl hover:shadow-indigo-500/20' : 'bg-white/80 border-gray-200/60 hover:border-gray-400/60 shadow-md hover:shadow-xl hover:shadow-blue-500/10'}`} onClick={() => navigate(`/blog/${post.slug}`)}>
                              <CardContent className="p-0 h-full flex flex-col">
                                ...
                                {post.banner && <div className={`relative overflow-hidden rounded-t-lg flex-shrink-0 ${isTablet ? 'h-24' : 'h-32'}`}>
