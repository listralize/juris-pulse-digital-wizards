...
const NavigationItem = ({ to, label, isActive, onClick }: NavigationItemProps) => {
  ...
  return (
    <Link
      ...
      className={`relative px-3 py-2 font-medium transition-all duration-300 text-sm group focus:outline-none ${
        isActive
          ? (isDark ? 'text-white' : 'text-black')
          : (isDark ? 'text-white/70 hover:text-white focus:text-white' : 'text-black/70 hover:text-black focus:text-black')
      }`}
