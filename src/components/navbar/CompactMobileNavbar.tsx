...
const CompactMobileNavbar = ({ showLogo = true }: CompactMobileNavbarProps) => {
  ...
  return (
    <>
      <nav className={`md:hidden fixed top-0 left-0 right-0 ${isDark ? 'bg-black/95' : 'bg-white/95'} backdrop-blur-md border-b ${isDark ? 'border-neutral-800' : 'border-neutral-200'} transition-colors duration-300`} style={{ position: 'fixed', zIndex: 99999999, pointerEvents: 'auto' }}>
        ...
        {/* Mobile Menu */}
        {isMenuOpen && (
          <div className={`${isDark ? 'bg-black border-neutral-800' : 'bg-white border-neutral-200'} border-t`} style={{ zIndex: 99999999, position: 'relative' }}>
