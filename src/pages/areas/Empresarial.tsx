...
const EmpresarialPage = () => {
  ...
  return <PracticeAreaLayout title="Direito Empresarial" description="No tabuleiro de xadrez do mundo corporativo, onde cada movimento pode definir o destino de um império, a mediocridade não é uma opção. Empresas não buscam apenas advogados; buscam estrategistas, negociadores implacáveis e parceiros que transformam desafios em vitórias." currentArea="empresarial">
      <div className="mb-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {services.map((service, index) => <Card key={index} className={`${isDark ? 'bg-black/80 border-white/10' : 'bg-white/80 border-black/10'} border hover:${isDark ? 'bg-black/60' : 'bg-white/60'} transition-all duration-300 cursor-pointer group`} onClick={() => navigate(service.path)}>
      ...
      <div className="mt-16 p-8 rounded-xl bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border border-blue-200 dark:border-blue-800">
        <h3 className={`text-2xl font-canela mb-4 ${isDark ? 'text-white' : 'text-black'}`}>Nossa Abordagem: </h3>
