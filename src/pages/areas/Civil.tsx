...
const CivilPage = () => {
  ...
  return <PracticeAreaLayout title="Direito Civil" description="A essência da advocacia constitucional e administrativa onde a maestria não se mede apenas pelo conhecimento, mas pela capacidade de antecipar movimentos, proteger ativos e garantir a vitória. Uma estratégia de blindagem e expansão para os interesses mais vitais." currentArea="civil">
      <div className="space-y-16">
        ...
        {serviceCategories.map((category, categoryIndex) => <div key={categoryIndex} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {category.services.map((service, serviceIndex) => <Card key={serviceIndex} className={`${isDark ? 'bg-black/80 border-white/10' : 'bg-white/80 border-black/10'} border hover:${isDark ? 'bg-black/60' : 'bg-white/60'} transition-all duration-300 cursor-pointer group`} onClick={() => navigate(service.path)}>
